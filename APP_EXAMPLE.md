...
Тап по кнопке `Get push info` выполняет [проверку возможности получения push-уведомления](https://www.rustore.ru/help/sdk/push-notifications/checkpushavailability). Результат проверки выводится в лог на экране.
...
Тап по кнопке `Get push token` выполняет процедуру [получения текущего push-токена пользователя](https://www.rustore.ru/help/sdk/push-notifications/token-methods). Push-токен пользователя, полученный в приложении, используется в структуре `message` для [отправки push-уведомлений](https://www.rustore.ru/help/sdk/push-notifications/send-push-notifications).
